...
public class StringUtil implements PLog {
   ...
   public static String renderingPaging(int maxNum,int currentPageNo, int rowPerPage, int bottomCount,String url, String scriptName) {
	 ...
	 * @return
	 */
	public static String getCodeList(List<CodeVO> pageCode, String selectedVal) {
		StringBuilder sb=new StringBuilder(300);
		
        if(null != pageCode && pageCode.size()>0){
        	for(CodeVO code :pageCode){     
        		String optionValue = "";   		
        		
        		if(code.getDetCode().equals(selectedVal)) {
        			optionValue = "<option  value='"+code.getDetCode()+"'  selected>"+code.getDetCodeNm()+"</option>";
        		}else {
        			optionValue = "<option  value='"+code.getDetCode()+"'>"+code.getDetCodeNm()+"</option>";
        		}
